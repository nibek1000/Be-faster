...
public class steady : MonoBehaviour {
	...
	void Start () {
        ...
        FirstTime = PlayerPrefs.GetInt("ft2");
        tut = PlayerPrefs.GetInt("tut2");
        StartCoroutine(tut11());
        if (FirstTime == 1)
        {
            Combo = PlayerPrefs.GetInt("combo1");
            speed = PlayerPrefs.GetFloat("speed1");
        }
        else
        {
            FirstTime++;
            PlayerPrefs.SetInt("ft2", FirstTime);
        }
        if(tut == 0)
        {
            Tutorial.SetActive(true);
            tut = 1;
            PlayerPrefs.SetInt("tut2", tut);
        }
        StartCoroutine(Save());
	...
	void Update () {
        ...
        if(speed <= 0.8f)
        {
            Debug.Log("Unlocked");
            unlock.Unlocked = 1;
            PlayerPrefs.SetInt("rankedUnlock", 1);
    ...
    IEnumerator Ready()
    {
        ...
        mainText.text = "Ready";
        yield return new WaitForSeconds(rsg);
    ...
    IEnumerator Save()
    {
        ...
        PlayerPrefs.SetInt("combo1", Combo);
        StartCoroutine(Save());
        if (invalisSp == true)
        {
            Social.ReportScore(Combo, "CgkIm6eM5P4ZEAIQLg", (bool success) =>
            {
                // handle success or failure
            });
            Social.ReportScore(leaderboardSpeed, "CgkIm6eM5P4ZEAIQLQ", (bool success) =>
            {
                // handle success or failure
            });
        }
